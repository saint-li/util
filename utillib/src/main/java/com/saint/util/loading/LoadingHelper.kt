...
class LoadingHelper @JvmOverloads constructor(private val contentView: View, contentAdapter: ContentAdapter<*>? = null) {
    ...
    fun skeleton() = showView(ViewType.SKELETON)

    fun content() = showView(ViewType.CONTENT)

    fun error() = showView(ViewType.ERROR)

    fun empty() = showView(ViewType.EMPTY)
    ...
    fun showView(viewType: Any) {
        if (currentViewHolder == null) {
            ...
            if (viewType !== currentViewHolder!!.viewType
                && currentViewHolder!!.rootView.parent != null
            ) {
                contentParent.removeView(currentViewHolder!!.rootView)
                addView(viewType)
            }
...
enum class ViewType {
    TITLE, LOADING, CONTENT, ERROR, EMPTY, SKELETON
}