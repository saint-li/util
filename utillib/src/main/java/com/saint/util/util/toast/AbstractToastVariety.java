...
abstract class AbstractToastVariety {
    ...
    public static final int TOAST_TAG_DARK = -3;
    ...
    public AbstractToastVariety(int tag) {
        ...
        AppLog.d("toast variety" + AppUtil.getObjectDesc(this) + "has created");
    }
    ...
    protected void show() {
        if (AppUtil.isNotificationPermitted()) {
            mToast.show();
        } else {
            VirtualToastManager.get().show(mToast, mWindowParams);
        }
        mLastShowTime = System.currentTimeMillis();
        AppLog.d("show toast" + AppUtil.getObjectDesc(mToast));
    }
    ...
    protected void rebuildToast() {
        dismiss();
        mToast = AppUtil.requireNonNull(createToast(), "createToast can not return null!");
        mView = mToast.getView();
        retrieveMsgView();
        mVerticalAxisOffsetWhenBottom = mYOffset = mToast.getYOffset();
        mVerticalAxisOffsetWhenTop = AppUtil.getToolbarHeight() + AppUtil.dpToPx(40);
        setupReflectInfo();
        AppLog.d("rebuild toast" + AppUtil.getObjectDesc(mToast));
    }
    ...
    public boolean isShowing() {
        boolean isShowing = AppUtil.isNotificationPermitted() ? mToast != null && mToast.getView().getWindowVisibility() == View.VISIBLE
