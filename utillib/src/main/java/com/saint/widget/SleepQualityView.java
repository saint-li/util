...
public class SleepQualityView extends View {
    ...
    /* ********************* 外部设置的属性 *********************** */
    //颜色属性
    private int deepColor = Color.parseColor("#9272D1");//深度睡眠的颜色
    private int shallowColor = Color.parseColor("#A4A7FA");//浅度睡眠的颜色
...
//    private float widthRatio = 0.05f;//线条宽度百分比
    ...
    //    private float sleepInterval = AppUtil.getDimen(R.dimen.dp_20);
    //画板四周边距
...
//            float canvasHeight = (1f - marginTopRatio - marginBottomRatio - xTextRatio) * getHeight();
            wakeYAxis = getPaddingTop();
            shallowYAxis = getPaddingTop();
            deepYAxis = getHeight() - (sleepHeight * 2);
            AppLog.e("布局高度：" + getHeight() + "   等分：" + sleepHeight
                     + "  浅睡：" + shallowYAxis + "  深睡：" + deepYAxis);
            //画y坐标
...
//                linePaint.setStrokeWidth(widthRatio * getHeight());
                for (int i = 0; i < timeArray.size(); i++) {
                    ...
                    if (selectValue.length >= 3) {
                        ...
                        //设置横线宽度
                        linePaint.setStrokeWidth(sleepHeight * 2);
...
//            sleepHeight = typedArray.getFloat(R.styleable.SleepQualityView_sq_sleepHeight, sleepHeight);
...
//        linePaint.setStrokeCap(Paint.Cap.ROUND); // 把每段圆弧改成圆角的
        ...
        timeArray.add(new float[]{2f, 0.0f, 0.05f});
        timeArray.add(new float[]{3f, 0.05f, 0.1f});
        timeArray.add(new float[]{2f, 0.15f, 0.05f});
        timeArray.add(new float[]{3f, 0.2f, 0.2f});
        timeArray.add(new float[]{2f, 0.4f, 0.05f});
        timeArray.add(new float[]{3f, 0.45f, 0.1f});
        timeArray.add(new float[]{2f, 0.55f, 0.05f});
        timeArray.add(new float[]{3f, 0.6f, 0.15f});
        timeArray.add(new float[]{2f, 0.75f, 0.25f});
...
//    }
     ...
     * 设置横线的宽度
     */
    public void setWidthRatio(float sleepHeight) {
        if (this.sleepHeight != sleepHeight) {
            this.sleepHeight = sleepHeight;
            invalidate();
        }
    }
